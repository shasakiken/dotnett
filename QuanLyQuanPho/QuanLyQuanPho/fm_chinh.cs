...
namespace QuanLyQuanPho
{
    public partial class fm_chinh : Form
    {
        ...
        public fm_chinh(Account acc)
        {
           ...
           // DataTable taikhoan = DataProvider.Instance.ExecuteQuery("SELECT Type fROM dbo.Account");
            
            //if (taikhoan.Row == 0)
              //  chủQuánToolStripMenuItem.Visible = false;

        }

        void ChangeAccount(int type)
        {
            menuchuquan.Enabled = type == 1;
            thôngToolStripMenuItem.Text += " (" + LoginAccount.DisplayName + ")";
            if(type==0)
            menuchuquan.Visible = false;
        ...
        {
            fm_nhanvien f = new fm_nhanvien(LoginAccount);
            f.ShowDialog();
        }
